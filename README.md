...
* Main Features
  * Gossip Queen - Real Time Hottest Trends world wide
    ...
    2. Real Time Trends - The data source is twitter search API with "travel" as the search key words. I have cleaned the data. The tweets are sorted by retweet count in descending order. Origionally, I was using Python pretty table, but it bacame pretty slow when there were more tweets to deal with, especially when there is sorting. However, using Spark DataFrame became much faster, it will easier to do sorting, or other sql query in the future.
      ...
      It really takes time to do data process here. Need to choose the right data structure to store the extracted data, then when writing Saprk sql query, I did several tests to see how can I combine same retweets together and show the sum counts. We may need to take care of Python indent when using Saprk Cluster on the cloud, sometimes, the location/url data will be insterted into the count coulmn even when I didn't change anything. When I have seperate a longer code line into 2 lines, the output became normal.
